...
async function pagi(Fernazer, x, data) {
    ...
    return await Fernazer.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(Fernazer, x, data) {
    let row = [];
    let ment = [];
...
async function malam(Fernazer, x, data) {
...
async function skill(Fernazer, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await Fernazer.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, Fernazer, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await Fernazer.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
// playing
async function run(Fernazer, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Fernazer, data);
            break;
        } else {
            await sleep(90000);
        }
        if (getWinner(id, data).status != null) {
            ...
            await pagi(Fernazer, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Fernazer, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(Fernazer, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Fernazer, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(Fernazer, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Fernazer, data);
            break;
        } else {
            await skill(Fernazer, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, Fernazer, data);
}

async function run_vote(Fernazer, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Fernazer, data);
            break;
...
async function run_malam(Fernazer, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Fernazer, data);
            break;
...
async function run_pagi(Fernazer, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, Fernazer, data);
            break;
